...
def execute():
	...
	if frappe_v.startswith("14") and hrms_v.startswith("15"):
		message = f"""
			The `develop` branch of Payroll Jamaica is no longer compatible with Frappe & ERPNext's `version-14`.
			Since you are using ERPNext/Frappe `version-14` please switch Payroll Jamaica's branch to `version-14` and then proceed with the update.\n\t
